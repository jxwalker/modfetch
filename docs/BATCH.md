...
- Generated by `modfetch pack export` for curated task packs and by
  `modfetch snapshot` for filtered Hugging Face repository subsets.
...
For small curated bundles:

```bash
modfetch pack export --id llm-smoke --output llm-smoke.yml
modfetch download --batch llm-smoke.yml
```

For a Hugging Face repository subset:

```bash
modfetch snapshot hf://owner/repo --include '*.gguf' --output repo-gguf.yml
modfetch download --batch repo-gguf.yml --batch-parallel 2
```
