...
func TestChunkedDownload1MB(t *testing.T) {
	...
	dest, sha, err := dl.Download(context.Background(), url, "", "")
...
func TestChunkedCorruptAndRepair(t *testing.T) {
	...
	dest, goodSHA, err := dl.Download(context.Background(), url, "", "")
	...
	_, fixedSHA, err := dl.Download(context.Background(), url, dest, goodSHA)
