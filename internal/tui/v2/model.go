	...
	case tea.MouseMsg:
		if m.newJob {
			...
			if msg.Action == tea.MouseActionPress && msg.Button == tea.MouseButtonLeft {
				trigger := tea.KeyEnter
				if msg.X < m.w/2 {
					trigger = tea.KeyEsc
				}
				return m, tea.Batch(cmd, func() tea.Msg { return tea.KeyMsg{Type: trigger} })
			}
			return m, cmd
		}
		if m.batchMode {
		...
		if msg.Action == tea.MouseActionPress {
			...
			case tea.MouseButtonLeft:
				row := msg.Y - m.tableYOffset()
				rows := m.visibleRows()
				max := m.maxRowsOnScreen()
				if row >= 0 && row < len(rows) && row < max {
					m.selected = row
				}
			}
		}
...
func (m *Model) tableYOffset() int {
	// Title bar, tabs, and commands each render with a border (3 lines each).
	// The table itself adds a top border and a header line before rows start.
	return 3*3 + 2
}
