...
func (m *Model) View() string {
	...
	welcome := ""
	if m.showWelcome {
		welcome = m.th.border.Width(m.w - 2).Render(m.renderWelcome())
	}
	modal := ""
	if m.newJob {
...
func (m *Model) loadUIState() uiState {
	p := m.uiStatePath()
	var st uiState
	b, err := os.ReadFile(p)
	if err != nil {
...
func (m *Model) saveUIState() {
	...
	st := uiState{ThemeIndex: m.themeIndex, ShowURL: m.columnMode == "url", ColumnMode: m.columnMode, Compact: m.cfg.UI.Compact, GroupBy: m.groupBy, SortMode: m.sortMode, FirstRun: !m.showWelcome}
...
func (m *Model) renderWelcome() string {
	var sb strings.Builder
	sb.WriteString(m.th.head.Render("Welcome to modfetch") + "\n")
	sb.WriteString("j/k navigate • n new download • ? help • q quit\n")
