...
func defaultTheme(tcfg config.Theme) Theme {
	...
	if tcfg.Border != "" {
		th.border = b.BorderForeground(lipgloss.Color(tcfg.Border))
	}
	if tcfg.Title != "" {
		th.title = th.title.Foreground(lipgloss.Color(tcfg.Title))
	}
	if tcfg.Label != "" {
		th.label = lipgloss.NewStyle().Faint(true).Foreground(lipgloss.Color(tcfg.Label))
	}
	if tcfg.TabActive != "" {
		th.tabActive = th.tabActive.Foreground(lipgloss.Color(tcfg.TabActive))
	}
	if tcfg.TabInactive != "" {
		th.tabInactive = th.tabInactive.Foreground(lipgloss.Color(tcfg.TabInactive))
	}
	if tcfg.Row != "" {
		th.row = th.row.Foreground(lipgloss.Color(tcfg.Row))
	}
	if tcfg.RowSelected != "" {
		th.rowSelected = th.rowSelected.Foreground(lipgloss.Color(tcfg.RowSelected))
	}
	if tcfg.Head != "" {
		th.head = th.head.Foreground(lipgloss.Color(tcfg.Head))
	}
	if tcfg.Footer != "" {
		th.footer = th.footer.Foreground(lipgloss.Color(tcfg.Footer))
	}
	if tcfg.OK != "" {
		th.ok = th.ok.Foreground(lipgloss.Color(tcfg.OK))
	}
	if tcfg.Bad != "" {
		th.bad = th.bad.Foreground(lipgloss.Color(tcfg.Bad))
	}
	return th
}
...
func New(cfg *config.Config, st *state.DB, version string) tea.Model {
	...
	m := &Model{
		cfg: cfg, st: st, th: defaultTheme(cfg.UI.Theme), activeTab: -1, prog: p, prev: map[string]obs{},
...
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "T": // theme cycle presets
			...
			cfgs := themePresetConfigs()
			m.themeIndex = (m.themeIndex + 1) % len(presets)
			m.th = presets[m.themeIndex]
			if m.themeIndex < len(cfgs) {
				m.cfg.UI.Theme = cfgs[m.themeIndex]
			}
			m.saveUIState()
			return m, nil
...
func (m *Model) loadUIState() {
	...
	if (st.Theme != config.Theme{}) {
		m.cfg.UI.Theme = st.Theme
	}
	if m.themeIndex >= 0 {
		presets := themePresets()
		cfgs := themePresetConfigs()
		if m.themeIndex < len(presets) {
			m.th = presets[m.themeIndex]
			if m.themeIndex < len(cfgs) {
				m.cfg.UI.Theme = cfgs[m.themeIndex]
			}
		...
		m.th = defaultTheme(m.cfg.UI.Theme)
	}
	if st.ColumnMode != "" {
...
func (m *Model) saveUIState() {
	...
	st := uiState{ThemeIndex: m.themeIndex, Theme: m.cfg.UI.Theme, ShowURL: m.columnMode == "url", ColumnMode: m.columnMode, Compact: m.cfg.UI.Compact, GroupBy: m.groupBy, SortMode: m.sortMode}
...
func themePresets() []Theme {
	base := defaultTheme(config.Theme{})
	neon := base
...
func themePresetConfigs() []config.Theme {
	return []config.Theme{
		{Border: "63", Title: "81", TabActive: "219", TabInactive: "240", Head: "213", OK: "42", Bad: "196"},
		{Border: "51", Title: "46", TabActive: "48", TabInactive: "240", Head: "49", OK: "42", Bad: "196"},
		{Border: "135", Title: "213", TabActive: "204", TabInactive: "240", Head: "219", OK: "42", Bad: "196"},
		{Border: "136", Title: "136", TabActive: "166", TabInactive: "240", Head: "178", OK: "42", Bad: "196"},
	}
}
