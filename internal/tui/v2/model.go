...
func (k keyMap) ShortHelp() []key.Binding {
	return []key.Binding{
		k.New, k.Batch, k.Start, k.Cancel, k.Delete, k.Open,
		k.Filter, k.SortSpeed, k.SortETA, k.SortClear,
		k.Group, k.Column, k.Compact, k.Inspector, k.Toasts, k.Help, k.Quit,
	}
}

func (k keyMap) FullHelp() [][]key.Binding {
	return [][]key.Binding{
		{k.Up, k.Down, k.Filter, k.Group, k.Column, k.Compact},
		{k.New, k.Batch, k.Start, k.Cancel, k.Delete, k.Open, k.Probe},
		{k.Select, k.SelectAll, k.SelectNone},
		{k.SortSpeed, k.SortETA, k.SortClear},
		{k.CopyPath, k.CopyURL, k.Inspector, k.Toasts, k.Help, k.Quit},
	}
}
...
var keys = keyMap{
	...
	Group:      key.NewBinding(key.WithKeys("g"), key.WithHelp("g", "group host")),
	Column:     key.NewBinding(key.WithKeys("t"), key.WithHelp("t", "cycle column")),
	Compact:    key.NewBinding(key.WithKeys("v"), key.WithHelp("v", "compact view")),
	New:        key.NewBinding(key.WithKeys("n"), key.WithHelp("n", "new job")),
	Batch:      key.NewBinding(key.WithKeys("b"), key.WithHelp("b", "batch import")),
	Start:      key.NewBinding(key.WithKeys("y", "r"), key.WithHelp("y/r", "start")),
	Cancel:     key.NewBinding(key.WithKeys("p"), key.WithHelp("p", "cancel")),
	...
	SelectNone: key.NewBinding(key.WithKeys("X"), key.WithHelp("X", "clear selection")),
	SortSpeed:  key.NewBinding(key.WithKeys("s"), key.WithHelp("s", "sort speed")),
	SortETA:    key.NewBinding(key.WithKeys("e"), key.WithHelp("e", "sort ETA")),
	SortClear:  key.NewBinding(key.WithKeys("o"), key.WithHelp("o", "clear sort")),
	...
	Inspector:  key.NewBinding(key.WithKeys("i"), key.WithHelp("i", "inspector")),
	Toasts:     key.NewBinding(key.WithKeys("H"), key.WithHelp("H", "toasts")),
...
func (m *Model) renderCommandsBar() string {
	m.help.Width = m.w - 4
	return m.th.footer.Render(m.help.ShortHelpView(m.keys.ShortHelp()))
}

func (m *Model) renderHelp() string {
	m.help.Width = m.w - 4
	return m.th.head.Render("Help") + "\n" + m.help.FullHelpView(m.keys.FullHelp())
}
