	...
	for _, rule := range cfg.Placement.Mapping {
		if rule.Match == artifactType {
			for _, t := range rule.Targets {
				...
				if !ok { return nil, fmt.Errorf("mapping references unknown app: %s", t.App) }
				rel, ok := app.Paths[t.PathKey]
				if !ok { return nil, fmt.Errorf("app %s missing path key: %s", t.App, t.PathKey) }
...
func Place(cfg *config.Config, srcPath, artifactType, mode string) ([]string, error) {
	if mode == "" { mode = cfg.General.PlacementMode }
	if artifactType == "" { artifactType = classifier.Detect(srcPath) }
	dests, err := ComputeTargets(cfg, artifactType)
	if err != nil { return nil, err }
	if len(dests) == 0 { return nil, fmt.Errorf("no mapping targets for type %s", artifactType) }
	...
	for _, dir := range dests {
		if err := os.MkdirAll(dir, 0o755); err != nil { return placed, err }
		...
		if exists(dst) {
			...
			if err := os.Remove(dst); err != nil && !os.IsNotExist(err) { return placed, err }
		}
		switch strings.ToLower(mode) {
		case "symlink":
			// Use relative symlink when possible
			rel, _ := filepath.Rel(dir, srcPath)
			if rel == "" || strings.HasPrefix(rel, "..") { rel = srcPath }
			if err := os.Symlink(rel, dst); err != nil { return placed, err }
		case "hardlink":
			if err := os.Link(srcPath, dst); err != nil { return placed, err }
		case "copy":
			if err := copyFile(srcPath, dst); err != nil { return placed, err }
		default:
